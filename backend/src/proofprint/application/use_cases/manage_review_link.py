...
class _ReviewLinkUseCase:
    def __init__(
        self, commands: WorkspaceCommandRepository, review_access: ReviewAccessRepository,
        links: ReviewLinkTokenCodec, unit_of_work: UnitOfWork, review_base_url: str,
    ) -> None:
    ...
    def _next_revision(self, workspace_id: UUID, expected_revision: int) -> int:
        state = self.commands.get_workspace_state_for_update(workspace_id)
        if state is None:
            raise ResourceNotFound("Workspace was not found")
        revision, record_status = state
        if revision != expected_revision:
            raise PreconditionFailed("Workspace revision does not match If-Match")
        if record_status != "ACTIVE":
            raise Conflict("Workspace must be ACTIVE to manage the review link")
        return revision + 1

    def _view(self, link: WorkspaceReviewLink) -> ReviewLinkView:
        return ReviewLinkView(
            link=link, review_url=f"{self.review_base_url}/review/{self.links.issue(link)}"
        )


class GetReviewLink(_ReviewLinkUseCase):
    def execute(self, actor: CurrentActor, workspace_id: UUID) -> ReviewLinkView:
...
class DisableReviewLink(_ReviewLinkUseCase):
    def execute(
        self, actor: CurrentActor, workspace_id: UUID, reason: str, expected_revision: int
    ) -> int:
        self._authorize(actor, workspace_id)
        next_revision = self._next_revision(workspace_id, expected_revision)
        ...
        try:
            ...
            self.commands.update_workspace_revision(workspace_id, next_revision, now)
            self.commands.add_audit_event(
                workspace_id=workspace_id, actor_id=actor.id,
                event_type="REVIEW_LINK_DISABLED", entity_type="WorkspaceReviewLink",
                entity_id=link.id,
                metadata={
                    "reason": reason.strip(), "revoked_sessions": revoked,
            ...
            if revoked:
                self.commands.add_audit_event(
                    workspace_id=workspace_id, actor_id=actor.id,
                    event_type="GUEST_SESSION_REVOKED", entity_type="WorkspaceReviewLink",
                    entity_id=link.id, metadata={"revoked_sessions": revoked},
                )
...
class RotateReviewLink(_ReviewLinkUseCase):
    def execute(
        self, actor: CurrentActor, workspace_id: UUID, reason: str, expected_revision: int
    ) -> tuple[ReviewLinkView, int]:
        self._authorize(actor, workspace_id)
        next_revision = self._next_revision(workspace_id, expected_revision)
        ...
        replacement = WorkspaceReviewLink(
            id=uuid4(), workspace_id=workspace_id, version=current.version + 1,
            status=ReviewLinkStatus.ACTIVE, created_by=actor.id, created_at=now,
        )
        try:
            ...
            self.commands.add_audit_event(
                workspace_id=workspace_id, actor_id=actor.id,
                event_type="REVIEW_LINK_ROTATED", entity_type="WorkspaceReviewLink",
                entity_id=replacement.id,
                metadata={
                    "previous_link_id": str(current.id), "link_version": replacement.version,
                    "reason": reason.strip(), "revoked_sessions": revoked,
                    "workspace_revision": next_revision,
                },
            )
            if revoked:
                self.commands.add_audit_event(
                    workspace_id=workspace_id, actor_id=actor.id,
                    event_type="GUEST_SESSION_REVOKED", entity_type="WorkspaceReviewLink",
                    entity_id=current.id, metadata={"revoked_sessions": revoked},
                )
            self.unit_of_work.commit()
        except Exception:
            self.unit_of_work.rollback()
            raise
        return self._view(replacement), next_revision
